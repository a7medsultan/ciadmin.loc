...
class RoleModel extends Model
{
    ...
    function getName($id)
    {
        $record = $this->select('name')->where('id', $id)->get()->getRow(); // Fetch the row corresponding to the given ID
        if ($record) {
    ...
    public function getDatatable($params = null)
    {
        $where = "";
        if ($params['search']) {
            $where = "name like '%{$params['search']}%' or created_at like '%{$params['search']}%'";
            $this->where($where);
        }
